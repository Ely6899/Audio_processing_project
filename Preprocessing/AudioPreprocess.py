...
def read_audio_file_as_waveform(file_name: str,
                                data_root_path: Path = LIBRISPEECH_TRAIN_ROOT_FOLDER,
                                sr: float | None = None) -> Optional[tuple[np.ndarray, int | float]]:
    """
    Reads the supported audio file as an array of amplitude values
    @rtype: Tuple
    @param file_name: Name of a supported audio file to read
